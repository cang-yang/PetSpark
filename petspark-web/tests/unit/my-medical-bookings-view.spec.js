...
import MyMedicalBookingsView from '@/views/MyMedicalBookingsView.vue'
import { listMyServiceBookings, getServiceBooking, cancelServiceBooking } from '@/api/service'

describe('MyMedicalBookingsView', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ...
    getServiceBooking.mockResolvedValue({
      data: { id: 'b-1', bookingNo: 'SVC-1', kind: 'MEDICAL', serviceItemName: '基础医疗', status: 'CONFIRMED', version: 1, customerName: '张三', customerPhone: '13800000000' }
    })
    cancelServiceBooking.mockResolvedValue({ data: { id: 'b-1', status: 'CANCELLED', version: 2 } })
  })

  it('loads only MEDICAL bookings on mount', async () => {
    const wrapper = shallowMount(MyMedicalBookingsView, {
    ...
    expect(listMyServiceBookings).toHaveBeenCalledWith({ kind: 'MEDICAL', status: undefined, page: 1, size: 10 })
